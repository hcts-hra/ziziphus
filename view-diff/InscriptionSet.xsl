            ...
            <tbody>
               <xsl:for-each select="vra:inscription">
                  <tr>
                     ...
                     <td>
                        <xsl:choose>
                           <xsl:when test="vra:text/@diff:attr-before-type or vra:text/@diff:attr-after-type">
                              <xsl:choose>
                                 <xsl:when test="string-length(string-join(vra:text/@diff:attr-before-type,'')) != 0">
                                    <div title="Type" class="diffs-attr-before" id="b-d2e407">
                                       <xsl:apply-templates select="vra:text/@diff:attr-before-type"></xsl:apply-templates>
                                    </div>
                                 </xsl:when>
                                 <xsl:otherwise>
                              ...
                              <xsl:choose>
                                 <xsl:when test="string-length(string-join(vra:text/@diff:attr-after-type,'')) != 0">
                                    <div title="Type" class="diffs-attr-after" id="b-d2e407">
                                       <xsl:apply-templates select="vra:text/@diff:attr-after-type"></xsl:apply-templates>
                                    </div>
                                 </xsl:when>
                                 <xsl:otherwise>
