<?xml version="1.0" encoding="UTF-8"?><xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:vra="http://www.vraweb.org/vracore4.htm" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:bfn="http://www.betterform.de/XSL/Functions" xmlns:xsd="http://www.w3.org/2001/XMLSchema" version="2.0">
   ...
   <xsl:template match="/vra:work |/vra:image">
      <xsl:variable name="side" select="if(local-name(.)='work') then 'leftPanel' else 'rightPanel'"></xsl:variable>
      <xsl:variable name="script" select="if(local-name(.)='work') then 'showWorkXML();' else 'showImageXML();'"></xsl:variable>
      <div class="columntitle">
         <xsl:value-of select="$title"></xsl:value-of>
         <xsl:text> </xsl:text><button type="button" class="showXML" id="{$recordType}-showXML" title="show raw XML" onclick="{$script}">XML</button><button type="button" class="historyButton" id="{$recordType}-versions-button" subject="{$recordId}" title="Versions history for {$recordType} {$recordId}">H</button></div>
