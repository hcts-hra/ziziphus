                     ...
                     <td>
                        <xsl:choose>
                           <xsl:when test="string-length(string-join(vra:term/@type,'')) != 0">
                              <div data-bf-type="select1" data-bf-bind="vra:term/@type" tabindex="0" title="Type" id="b-d2e1318">
                                 <xsl:value-of select="vra:term/@type"></xsl:value-of>
                              </div>
                           </xsl:when>
                           <xsl:otherwise>
                              <div class="nodata" data-bf-type="select1" data-bf-bind="vra:term/@type" tabindex="0">(Type)</div>
                           </xsl:otherwise>
                        </xsl:choose>
                        <xsl:choose>
                           <xsl:when test="string-length(string-join(vra:term,'')) != 0">
                              <div data-bf-type="input" data-bf-bind="vra:term" tabindex="0" title="Term" id="b-d2e1311">
